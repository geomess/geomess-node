...
GeoMessDAO.prototype.createAgent = function(appalias, username, agentName, type, longitude, latitude, timestamp, hashSubmitted, responseHash, response, bayeux){
	var self = this;
		...
		if(app==null){
			...
			db.collection('users').findOne({"username":username,"app":app._id}, function(err, user){
				if(user!=null){
					...
					var messageToHash = 'createAgent|'+agentName+'|'+type+'|'+longitude+'|'+latitude+'|'+timestamp;
				    
				    var hashCalculated = crypto.createHmac('md5', user.token).update(messageToHash).digest('hex');
				    ...
				    if(hashCalculated===hashSubmitted){
				    	
						db.collection('agents').insert(
				    			   { 
				    				   'name': agentName,
				    				   'user': user._id,
				    				   'status': '',
				    				   ...
				    				   if(err){
				   							...
				   							self.emit('error',"error: "+err.message, responseHash, response, bayeux);
				    				   }else{
				    					   self.emit('create-agent-success', appalias, records[0], responseHash, response, bayeux);
				    				   }
				    	});
					...
					self.emit('error',"authentication failure", responseHash ,response, bayeux);
...
GeoMessDAO.prototype.getAgents = function(appalias, username, status, centerLat, centerLon, radius, responseHash, response, bayeux){
	var self = this;
		...
		if(app==null){
			...
			db.collection('users').find({"app":app._id}, function(err, userCursor){
				...
				userCursor.toArray(function(err, users) {
					...
					//status, centerLat, centerLon, radius,
					var filter = {};
					...
					if(centerLat!=null && centerLon!=null && radius!=null){
						var radiusInRad = parseFloat(radius) / (6371 * Math.PI / 180.0);
						var centerFilter = [ [ parseFloat(centerLon), parseFloat(centerLat) ], radiusInRad ];
						console.log(" * centerFilter: ",centerFilter);
						filter["loc"] = { '$within' : { '$center' : centerFilter } };
					}
					console.log(" * filter: ",filter);
					//find all agents of users
					db.collection('agents').find(filter, function(err, cursor){
