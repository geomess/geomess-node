...
//update position
dbClient.on('update-position-response', function (appalias, agentid, latitude, longitude, distanceTraveled, kmh, responseHash, response, bayeux) {
...
//create agent
dbClient.on('create-agent-success', function (appalias, created, responseHash, response, bayeux) {

	response.end('{"response":"success", "message":"agent created", "agent":'+JSON.stringify(created)+'}');
...
function getAgents(){
    ...
    var latitude = url_parts.query.latitude;
    var longitude = url_parts.query.longitude;
    var radius = url_parts.query.radius;

    dbClient.getAgents(appalias, username, status, latitude, longitude, radius, null, this.res, null);
...
function createAgent(){
	//TODO: limit possibile types
	this.res.writeHead(200, { 'Content-Type': 'application/json' });
	var url_parts = url.parse(this.req.url, true);
    
    var appalias = url_parts.query.app;
    var agentName = url_parts.query.agentName;
    ...
    dbClient.createAgent(appalias, username, agentName, type, longitude, latitude, timestamp, hashSubmitted, null, this.res, bayeux);
...
var router = new director.http.Router({
    ...
    '/createAgent': { get:createAgent, post: createAgent }
});
